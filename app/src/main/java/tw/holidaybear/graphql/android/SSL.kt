        ...
        init {
            try {
                ...
                if (socket != null) {
                    ...
                    // - remove all SSL2 versions (especially SSLv3) because they're insecure now
                    val protocols = LinkedList<String>()
                    for (protocol in socket.supportedProtocols)
                        if (!protocol.toUpperCase().contains("SSL2"))
                            protocols.add(protocol)
                    SSL.protocols = protocols.toTypedArray()
                    ...
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                        ...
                        SSL.cipherSuites = preferredCiphers.toTypedArray()
                    }
                }
            } catch (e: IOException) {
